...
class MPC:
    ...
    def get_action_cost(self, ac_seqs):
        ...
        ac_seqs = np.transpose(ac_seqs, [1, 0, 2])  # (T_plan, popsize, action_dim)
        ...
        for t in range(self.plan_horizon):
            ...
            # ([popsize*num_particles, nS], [popsize, action_dim*T])
            next_obs = self.predict_next_state(cur_obs, cur_acs)
            if isinstance(next_obs, list):
                next_obs = np.stack(next_obs)
            delta_costs = self.obs_cost_fn(next_obs).reshape([popsize, -1]) 
            # + action_costs  # think: do we need action noise ???
    ...
    def predict_next_state_model(self, states, actions):
        ...
        Given a list of state action pairs, use a single learned dynamics model to predict the next state.
            :param states  : [self.popsize * self.num_particles * self.planHorizon, self.state_dim]
            :param actions : [self.popsize * self.planHorizon, self.action_dim]
        ...
        """
        TA: For predict_next_state_model, the input states shape will be different dependent on the choice of optimizers. 
        For CEM, it will be [self.num_particles*self.popsize, self.state_dim]. For random it will be [self.num_particles*self.popsize*self.max_iters, self.state_dim].
        states in predict_next_state_model is arranged in this configuration [part1, part2, part3, part4, part5, part6, part1, …] for population size times.
        """
        # expand actions to match states len
        states, actions = torch.tensor(states, dtype=torch.float).to(self.model.device), torch.tensor(actions, dtype=torch.float).to(self.model.device)
        actions = torch.tile(actions, [len(states)//len(actions), 1])   # repeat by particles
        ...
        next_states = states + distrib.rsample()
    ...
    def predict_next_state_gt(self, states, actions):
        ...
        for t in range(len(states)):
            #s, r, term, _ = self.env.step(states[t], actions[t])
            next_states.append(self.env.get_nxt_state(states[t], actions[t]))
        return next_states
        raise NotImplementedError
