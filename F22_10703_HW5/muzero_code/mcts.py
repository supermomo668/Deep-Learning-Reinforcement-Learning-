...
def select_action(config, num_moves, node, network, test=False):
    ...
    print(f"Select action| visit counts:{visit_counts}")
    t = config.visit_softmax_temperature_fn(num_moves=num_moves)
    action = softmax_sample(visit_counts, 0 if test else t)
    return action
...
def select_child(config, node, min_max_stats):
    ...
    for act, ch in node.children.items():
        score = ucb_score(config, node, ch, min_max_stats)
        #print(f"Action: {act}, score:{score}")
        if score >= best_score:
            best_score = score
            action, child = act, ch
    return action, child
...
def expand_node(node, actions, network, parent_state, parent_action):
    ...
    """
    # get hidden state representation
    value, r, pi_logits, h  = network.recurrent_inference(
...
def softmax_sample(visit_counts, temperature):
    ...
    """
    # YOUR CODE HERE
    visit_cts = np.array([visit_ct for visit_ct, _ in visit_counts])
    if temperature==0:
        return [a for _, a in visit_counts][np.argmax(visit_cts)]
    pr = np.power(visit_cts, 1/temperature)
    return np.random.choice([a for _, a in visit_counts], size=1, p=pr/pr.sum())[0]
