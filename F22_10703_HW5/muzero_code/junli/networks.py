...
def action_to_one_hot(action, action_space_size):
    ...
    # print(action)
    # for a in action:
    #     one.append([1 if i == a else 0 for i in range(action_space_size)])    
    # return np.array(one).reshape(1, -1)
...
class CartPoleNetwork(BaseNetwork):
    ...
    def _conditioned_hidden_state(self, hidden_state: np.array, action: int) -> np.array:
        ...
        """
        # print(action_to_one_hot(action, self.action_size).shape, hidden_state.shape, '///')
        # hidden_state = tf.reshape(hidden_state, (-1, 4))
...
def one_hot(actions):
    ...
    a = np.zeros((actions.size, actions.max() +1 ))
    a[np.arange(actions.size), actions] = 1
...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        loss = 0
        total_value_loss = 0
        # total_value_loss = tf.cast( total_value_loss, tf.float32) 
        total_reward_loss = 0
        total_policy_loss = 0
        ...
        hidden_representation, value, policy_logits = network.initial_model(np.stack(state_batch))
        ...
        total_value_loss += 0.25 * tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=target_value_batch, logits=value))
        total_policy_loss += tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=target_policy_batch, logits=policy_logits))
        ...
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            conditioned_hidden = tf.concat(
                        [hidden_representation, one_hot(actions_batch)], axis=1)
            ...
            # Add to total losses
            # lv, lp, lr = 0, 0, 0
            lv = 0.25 * tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=target_value_batch, logits=value))
            lp = tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=target_policy_batch, logits=policy_logits))
            lr = mse(target_reward_batch, reward)
            
            total_value_loss += lv
            total_policy_loss += lp
            total_reward_loss += lr

            l_step = lv + lp + lr
            scale_gradient(l_step, 1. / config.num_unroll_steps)
            ...
            loss += l_step
            # scale_gradient(loss, config.num_unroll_steps)
        print(loss.numpy())
