...
def select_child(config, node, min_max_stats):
    ...
    for a in node.children.keys():
        ucb = ucb_score(config, node, node.children[a], min_max_stats)
        if ucb > _max:
            _max = ucb
            action = a
            child = node.children[a]

    assert not (action is None and child is None), "Error"
...
def expand_root(node, actions, network, current_state):
    ...
    # print(current_state)
    value, _, policy_logits, hidden_representation = network.initial_inference(np.expand_dims(current_state, 0))
    # Extract softmax policy and set node.policy
    policy_prob = network._softmax(policy_logits)
    node.hidden_representation = hidden_representation
    node.expanded = True
    node.reward = 0
    for i, a in enumerate(actions):
        child = Node(policy_prob[0][i])

        assert not (a in node.children)
...
def softmax_sample(visit_counts, temperature):
    ...
    # YOUR CODE HERE
    visit_count = to_array([v[0] for v in visit_counts])
    actions = to_array([v[1] for v in visit_counts])

    if temperature == 0:
        return actions[np.argmax(visit_count)]
    action_dist = visit_count ** (1 / temperature)
    action_dist /= np.sum(action_dist)

    return np.random.choice(actions, p=action_dist)
    # raise NotImplementedError()
