...
class Game:
    ...
    def store_search_statistics(self, root: Node):
        ...
        """
        sum_visits = sum(child.visit_count for 
                         child in root.children.values())
        self.child_visits.append(
            np.array([
                root.children[a].visit_count/ sum_visits if a in root.children else 0 for a in range(self.action_space_size)
            ])
        )
    ...
    def make_target(self, state_index, num_unroll_steps, td_steps):
        ...
        # Targets: [(value, last_reward, [np.array]), ...]
        # actions: [...]
