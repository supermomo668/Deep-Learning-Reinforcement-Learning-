...
class CartPoleNetwork(BaseNetwork):
    ...
    def _conditioned_hidden_state(self, hidden_state: np.array, action: int) -> np.array:
        ...
        conditioned_hidden = tf.concat(
            (hidden_state, action_to_one_hot(action, self.action_size)), 
            axis=1)
    ...
    def cb_get_variables(self) -> Callable:
        ...
        def get_variables():
            networks = (self.representation_network, self.value_network, 
                        self.policy_network,
                        self.dynamic_network, self.reward_network)
            return [variables
                    for variables_list in 
                    map(lambda n: n.trainable_weights, networks)
...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        # YOUR CODE HERE: Perform initial embedding of state batch
            # (value, reward, policy logits, hidden emb)
        
        h, v_init, pi_logits = network.initial_model(np.stack(state_batch))
        ...
        h = scale_gradient(h, 1/2)
        v_loss = scale_gradient(ce_criterion(target_value_batch, v_init), 1/4)
        pi_loss = ce_criterion(target_policy_batch, pi_logits)
        total_value_loss += v_loss
        total_policy_loss += pi_loss
        loss += v_loss + pi_loss
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            # use _conditioned_hidden_state row-by-row
            h_con = np.apply_along_axis(
                lambda x: tf.squeeze(network._conditioned_hidden_state(
                    np.expand_dims(x[:-1], 0), x[-1]), 0), 
                    axis=1, arr=tf.concat(
                        (h, np.expand_dims(np.array(actions_batch), 1)), axis=1))
            #h_con = network._conditioned_hidden_state(h, actions_batch)
            ...
            # Remember to scale value loss!
            v_loss = scale_gradient(ce_criterion(target_value_batch, v), 1/4)
            ...
            total_loss = v_loss + r_loss + pi_loss
            total_loss = scale_gradient(total_loss, 1/config.num_unroll_steps)
            loss +=  total_loss
        train_results.total_losses.append(tf.math.reduce_mean(loss).numpy())
        train_results.value_losses.append(tf.math.reduce_mean(total_value_loss).numpy())
        train_results.policy_losses.append(tf.math.reduce_mean(total_policy_loss).numpy())
        train_results.reward_losses.append(total_reward_loss.numpy())
        print(f"Loss:{train_results.total_losses[-1]}")
        return loss
