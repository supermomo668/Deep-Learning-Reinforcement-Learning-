...
def self_play(env, config: MuZeroConfig, replay_buffer: ReplayBuffer, network: CartPoleNetwork):
    ...
    games_played = 0
    test_rewards = TestResults('figures/')
    train_results = TrainResults('figures/')
    for i in range(config.num_epochs):  # Number of Steps of train/play alternations
        ...
        if reward_avg >= CARTPOLE_STOP_REWARD:
            ...
            # Plotting code here
            train_results.plot_individual_losses(name=f'END;epoch{i}')
            train_results.plot_total_loss(name=f'END;epoch{i}')
            test_rewards.plot_rewards(name=f'END;epoch{i}')
            return
        if i % 1 == 0:
            train_results.plot_individual_losses(name=f'epoch{i}')
            train_results.plot_total_loss(name=f'epoch{i}')
            test_rewards.plot_rewards(name=f'epoch{i}')
    # Plotting code here
    train_results.plot_individual_losses(name=f'END;epoch{i}')
    train_results.plot_total_loss(name=f'END;epoch{i}')
    test_rewards.plot_rewards(name=f'END;epoch{i}')
