...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        v_loss = 1/2*tf.math.reduce_mean(ce_criterion(target_value_batch, v_init))
        ...
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            # Remember to scale value loss!
            v_loss = 1/2*tf.math.reduce_mean(ce_criterion(target_value_batch, v))
            ...
            loss +=  total_loss
        train_results.total_losses.append(loss.numpy())
